...
export default function BookingPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-md mx-auto px-6 py-20 text-center">
        ...
        <p className="text-gray-500 mb-6">预订房间需要登录账号</p>
  ...
  if (loading) return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <Breadcrumb items={[{ label: '首页', href: '/' }, { label: '预订房间' }]} />
      <DetailSkeleton />
    </div>
  );
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        <div className="text-center py-20">
          ...
          <Button variant="outline" className="h-11 rounded-xl active:scale-[0.98] transition-all" onClick={() => navigate('/rooms')}>返回客房列表</Button>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.checkInDate || !form.checkOutDate || nights <= 0) {
      toast.error('请选择有效的入住和退房日期');
      return;
    }
    if (hasDateConflict) {
      toast.error('所选日期包含已被预订的日期，请重新选择');
      return;
    }
    setSubmitting(true);
    try {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <Breadcrumb items={[{ label: '首页', href: '/' }, { label: '预订房间' }]} />
      <h1 className="text-3xl font-bold tracking-tight mb-6">预订房间</h1>
        ...
        <CardContent className="pt-4">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <p className="text-sm text-gray-500">每晚</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-sm text-gray-500">最多{room.roomType?.maxGuests ?? 1}人</p>
        </div>
        <div className="space-y-2">
          <Label>特殊要求（选填）</Label>
          <Textarea value={form.specialRequests} onChange={e => setForm({ ...form, specialRequests: e.target.value })} placeholder="如有特殊要求请填写" className="focus:ring-2 focus:ring-amber-500" />
        </div>

        {hasDateConflict && (
          <p className="text-sm text-red-500">所选日期范围内包含已预订日期，请重新选择</p>
        )}
        ...
        {nights > 0 && (
          <Card className="rounded-2xl shadow-sm bg-amber-50 border-amber-200">
            <CardContent className="pt-4">
              <div className="flex justify-between items-center">
                <span>{nights}晚 × {formatPrice(room.roomType?.basePrice ?? 0)}/晚</span>
                <span className="text-2xl font-bold text-amber-600">总计: {formatPrice(totalPrice)}</span>
              </div>
            </CardContent>
          </Card>
        )}
