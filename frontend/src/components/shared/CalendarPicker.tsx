  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <button type="button" onClick={prevMonth} className="p-2 hover:bg-[#F3F1EC] rounded-xl transition-colors"><ChevronLeft className="w-5 h-5" /></button>
        <span className="text-base font-semibold">{year}年{month + 1}月</span>
        <button type="button" onClick={nextMonth} className="p-2 hover:bg-[#F3F1EC] rounded-xl transition-colors"><ChevronRight className="w-5 h-5" /></button>
      </div>
      <div className="grid grid-cols-7 gap-1">
        {DAYS.map(d => <div key={d} className="text-center text-sm text-[#8A8278] py-2">{d}</div>)}
        {cells.map((cell, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                ...
                cell.current && !disabled && 'hover:bg-[#F3F1EC] cursor-pointer',
                cell.current && disabled && !selected && 'text-gray-300 cursor-not-allowed line-through',
                selected && 'bg-[#C5A54E] text-white font-semibold',
                cell.current && !disabled && !selected && avail === false && 'bg-red-50 text-red-300 cursor-not-allowed line-through',
                cell.current && !disabled && !selected && avail === true && 'bg-[#C5A54E]/5 hover:bg-[#C5A54E]/10 text-[#C5A54E]',
              )}
            >
              {cell.day}
            </button>
      ...
      {availability && (
        <div className="flex items-center gap-4 mt-3 text-xs text-[#8A8278]">
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#C5A54E] inline-block"></span> 已选</span>
          ...
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#C5A54E]/5 border border-[#C5A54E]/20 inline-block"></span> 可选</span>
        </div>
      )}
    </div>
  );
